...
Este documento explica cómo está organizado `claude-brain` y cómo Claude Code
lo consume en la práctica. No es documentación de un sistema de software —
es la "arquitectura de conocimiento" del repo.
...
3. **Prioridad**: cuando el repo de trabajo (cliente) tiene su propio
   ...
   genérico de `claude-brain`. Este repo es la base por defecto, no un
...
`claude-brain` es agnóstico de proyecto: no sabe (ni debe saber) qué
...
convención de commits) se guarda en `.claude/ticket-workflow.config.json`
**dentro de cada repo de trabajo**, no en `claude-brain`. La skill en sí
(el comportamiento, el flujo, las preguntas) es lo único que vive acá.
